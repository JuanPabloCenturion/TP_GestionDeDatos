USE [GD2C2021]
GO

IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES
			WHERE TABLE_SCHEMA = 'LA_LOLA_ROTICERIA'))

BEGIN
	DROP TABLE LA_LOLA_ROTICERIA.MATERIAL
	DROP TABLE LA_LOLA_ROTICERIA.MATERIAL_X_TAREA
	DROP TABLE LA_LOLA_ROTICERIA.PAQUETE
	DROP TABLE LA_LOLA_ROTICERIA.TAREA_ASIGNADA
	DROP TABLE LA_LOLA_ROTICERIA.TAREA
	DROP TABLE LA_LOLA_ROTICERIA.TIPO_TAREA
	DROP TABLE LA_LOLA_ROTICERIA.ORDEN_TRABAJO
	DROP TABLE LA_LOLA_ROTICERIA.MECANICO
	DROP TABLE LA_LOLA_ROTICERIA.TALLER
	DROP TABLE LA_LOLA_ROTICERIA.ESTADO
	DROP TABLE LA_LOLA_ROTICERIA.VIAJE
	DROP TABLE LA_LOLA_ROTICERIA.RECORRIDO
	DROP TABLE LA_LOLA_ROTICERIA.CIUDAD
	DROP TABLE LA_LOLA_ROTICERIA.CHOFER
	DROP TABLE LA_LOLA_ROTICERIA.TIPO_PAQUETE
	DROP TABLE LA_LOLA_ROTICERIA.CAMION
	DROP TABLE LA_LOLA_ROTICERIA.MODELO_CAMION
	DROP TABLE LA_LOLA_ROTICERIA.MARCA
END

IF EXISTS (SELECT * FROM sys.schemas WHERE name = 'LA_LOLA_ROTICERIA')
BEGIN 
	DROP SCHEMA LA_LOLA_ROTICERIA; 
END


-- CREACION DEL ESQUEMA
GO
CREATE SCHEMA LA_LOLA_ROTICERIA
GO

CREATE TABLE LA_LOLA_ROTICERIA.MARCA(
	ID INT PRIMARY KEY IDENTITY(1,1),
	NOMBRE NVARCHAR(255)
)

CREATE TABLE LA_LOLA_ROTICERIA.MATERIAL(
	ID NVARCHAR(100) PRIMARY KEY,
	DESCRIPCION NVARCHAR(255),
	PRECIO DECIMAL(18,2)
)

CREATE TABLE LA_LOLA_ROTICERIA.TIPO_TAREA(
	ID INT PRIMARY KEY IDENTITY(1,1),
	TIPO NVARCHAR(255)
)

CREATE TABLE LA_LOLA_ROTICERIA.TAREA(
	ID INT PRIMARY KEY IDENTITY(1,1),
	CODIGO INT,
	DESCRIPCION NVARCHAR(255),
	TIPO_ID INT REFERENCES LA_LOLA_ROTICERIA.TIPO_TAREA(ID)
)

CREATE TABLE LA_LOLA_ROTICERIA.MATERIAL_X_TAREA(
	MATERIAL_ID nvarchar(100),
	TAREA_ID INT,
	CANTIDAD INT
)

CREATE TABLE LA_LOLA_ROTICERIA.CIUDAD(
	ID INT PRIMARY KEY IDENTITY(1,1),
	NOMBRE NVARCHAR(255)
)

CREATE TABLE LA_LOLA_ROTICERIA.ESTADO(
	ID INT PRIMARY KEY IDENTITY(1,1),
	DESCRIPCION NVARCHAR(255)
)

CREATE TABLE LA_LOLA_ROTICERIA.RECORRIDO(
	ID INT PRIMARY KEY IDENTITY(1,1),
	CIUDAD_ORIGEN INT REFERENCES LA_LOLA_ROTICERIA.CIUDAD(ID),
	CIUDAD_DESTINO INT REFERENCES LA_LOLA_ROTICERIA.CIUDAD(ID),
	KM_RECORIDOS INT,
	PRECIO_RECORRIDO DECIMAL(18,2)
)

CREATE TABLE LA_LOLA_ROTICERIA.MODELO_CAMION(
	ID INT PRIMARY KEY IDENTITY(1,1),
	DESCRIPCION NVARCHAR(255),
	VELOCIDAD_MAXIMA INT,
	CAPACIDAD_TANQUE INT,
	CAPACIDAD_CARGA INT,
	MARCA_ID INT REFERENCES LA_LOLA_ROTICERIA.MARCA(ID)
)

CREATE TABLE LA_LOLA_ROTICERIA.CAMION(
	PATENTE NVARCHAR(255) PRIMARY KEY,
	NRO_CHASIS NVARCHAR(255), 
	NRO_MOTOR NVARCHAR(255),
	FECHA_ALTA DATETIME2(3),
	MODELO_ID INT REFERENCES LA_LOLA_ROTICERIA.MODELO_CAMION(ID)
)

CREATE TABLE LA_LOLA_ROTICERIA.CHOFER(
	NRO_LEGAJO INT PRIMARY KEY,
	NOMBRE NVARCHAR(255),
	APELLIDO NVARCHAR(255),
	DNI DECIMAL(18,2), 
	DIRECCION NVARCHAR(255),
	TELEFONO INT,
	MAIL NVARCHAR(255),
	FECHA_NACIMIENTO DATETIME2(3),
	COSTO_HORA INT
)

CREATE TABLE LA_LOLA_ROTICERIA.VIAJE(
	VIAJE_NRO INT PRIMARY KEY IDENTITY(1,1),
	CAMION_ID NVARCHAR(255) REFERENCES LA_LOLA_ROTICERIA.CAMION(PATENTE),
	CHOFER_ID INT REFERENCES LA_LOLA_ROTICERIA.CHOFER(NRO_LEGAJO), 
	FECHA_INICIO DATETIME2(7),
	FECHA_FIN DATETIME2(3),
	CONSUMO_COMBUSTIBLE DECIMAL(18,2),
	RECORIDO_ID INT REFERENCES LA_LOLA_ROTICERIA.RECORRIDO(ID)
)

CREATE TABLE LA_LOLA_ROTICERIA.TIPO_PAQUETE(
	ID INT PRIMARY KEY IDENTITY(1,1),
	DESCRIPCION NVARCHAR(255),
	ANCHO_MAXIMO DECIMAL(18,2),
	ALTO_MAXIMO DECIMAL(18,2),
	LARGO_MAXIMO DECIMAL(18,2),
	PESO_MAXIMO DECIMAL(18,2),
	PRECIO DECIMAL(18,2)
)

CREATE TABLE LA_LOLA_ROTICERIA.PAQUETE(
	ID INT PRIMARY KEY IDENTITY(1,1),
	VIAJE_ID INT REFERENCES LA_LOLA_ROTICERIA.VIAJE(VIAJE_NRO),
	PRECIO_FINAL DECIMAL(18,2),
	CANTIDAD_PAQUETES INT,
	TIPO_ID INT REFERENCES LA_LOLA_ROTICERIA.TIPO_PAQUETE(ID)
)

CREATE TABLE LA_LOLA_ROTICERIA.ORDEN_TRABAJO(
	NRO INT PRIMARY KEY IDENTITY(1,1),
	CAMION_ID NVARCHAR(255) REFERENCES LA_LOLA_ROTICERIA.CAMION(PATENTE),
	FECHA_GENERACION NVARCHAR(255),
	ESTADO_ID INT REFERENCES LA_LOLA_ROTICERIA.ESTADO(ID)
)

CREATE TABLE LA_LOLA_ROTICERIA.TALLER(
	ID INT PRIMARY KEY IDENTITY(1,1),
	DIRECCION NVARCHAR(255),
	TELEFONO DECIMAL(18,0),
	MAIL NVARCHAR(255),
	NOMBRE NVARCHAR(255),
	CIUDAD_ID INT REFERENCES LA_LOLA_ROTICERIA.CIUDAD(ID)
)

CREATE TABLE LA_LOLA_ROTICERIA.MECANICO(
	NRO_LEGAJO INT PRIMARY KEY,
	TALLER_ID INT REFERENCES LA_LOLA_ROTICERIA.TALLER(ID),
	NOMBRE NVARCHAR(255),
	APELLIDO NVARCHAR(255),
	DNI DECIMAL(18,2),
	DIRECCION NVARCHAR(255),
	TELEFONO INT,
	MAIL NVARCHAR(255),
	FECHA_NACIMIENTO DATETIME2(3),
	COSTO_HORA INT
)

CREATE TABLE LA_LOLA_ROTICERIA.TAREA_ASIGNADA(
	ID INT PRIMARY KEY IDENTITY(1,1),
	MECANICO_ID INT REFERENCES LA_LOLA_ROTICERIA.MECANICO(NRO_LEGAJO),
	TAREA_ID INT REFERENCES LA_LOLA_ROTICERIA.TAREA(ID),
	TIEMPO_DE_EJECUCION_ESTIMADO INT,
	TIEMPO_DE_EJECUCION_REAL INT,
	FECHA_INICIO_ESTIMADA DATETIME2(3),
	FECHA_INICIO_REAL DATETIME2(3),
	FECHA_FIN DATETIME2(3),
	ORDEN_TRABAJO_ID INT REFERENCES LA_LOLA_ROTICERIA.ORDEN_TRABAJO(NRO)
)



-------------- MIGRACION --
--CIUDAD
INSERT INTO LA_LOLA_ROTICERIA.CIUDAD (NOMBRE)
	SELECT distinct RECORRIDO_CIUDAD_DESTINO ciudad
	FROM gd_esquema.Maestra
	WHERE RECORRIDO_CIUDAD_DESTINO is not null
	UNION
	SELECT distinct RECORRIDO_CIUDAD_ORIGEN ciudad
	FROM gd_esquema.Maestra
	WHERE RECORRIDO_CIUDAD_ORIGEN is not null

--RECORRIDO
INSERT INTO LA_LOLA_ROTICERIA.RECORRIDO (CIUDAD_ORIGEN, CIUDAD_DESTINO, KM_RECORIDOS, PRECIO_RECORRIDO)
	SELECT distinct (SELECT c.ID FROM LA_LOLA_ROTICERIA.CIUDAD c WHERE c.NOMBRE = RECORRIDO_CIUDAD_ORIGEN), 
	       (SELECT c.ID FROM LA_LOLA_ROTICERIA.CIUDAD c WHERE c.NOMBRE = RECORRIDO_CIUDAD_DESTINO), 		   
		   RECORRIDO_KM, 
		   RECORRIDO_PRECIO
	FROM gd_esquema.Maestra
	WHERE RECORRIDO_CIUDAD_DESTINO is not null
	or RECORRIDO_CIUDAD_ORIGEN is not null
	or RECORRIDO_PRECIO is not null

--MARCA
INSERT INTO LA_LOLA_ROTICERIA.MARCA (NOMBRE)
	SELECT distinct MARCA_CAMION_MARCA
	FROM gd_esquema.Maestra
	WHERE MARCA_CAMION_MARCA is not null

--MODELO CAMION
INSERT INTO LA_LOLA_ROTICERIA.MODELO_CAMION (DESCRIPCION, CAPACIDAD_CARGA, CAPACIDAD_TANQUE, VELOCIDAD_MAXIMA, MARCA_ID)	
	SELECT MODELO_CAMION, MODELO_CAPACIDAD_CARGA, MODELO_CAPACIDAD_TANQUE, MODELO_VELOCIDAD_MAX, (select ID from LA_LOLA_ROTICERIA.MARCA m where m.NOMBRE =  MARCA_CAMION_MARCA)
	FROM gd_esquema.Maestra
	WHERE MODELO_CAMION is not null
	GROUP BY MODELO_CAMION, MODELO_CAPACIDAD_CARGA, MODELO_CAPACIDAD_TANQUE, MODELO_VELOCIDAD_MAX, MARCA_CAMION_MARCA

--CAMION
INSERT INTO LA_LOLA_ROTICERIA.CAMION (PATENTE, NRO_CHASIS, NRO_MOTOR, FECHA_ALTA, MODELO_ID)	
	SELECT distinct CAMION_PATENTE, CAMION_NRO_CHASIS, CAMION_NRO_MOTOR, CAMION_FECHA_ALTA, (select m.ID from LA_LOLA_ROTICERIA.MODELO_CAMION m where m.DESCRIPCION = MODELO_CAMION and m.CAPACIDAD_CARGA = MODELO_CAPACIDAD_CARGA)
	FROM gd_esquema.Maestra
	WHERE CAMION_NRO_CHASIS is not null

--CHOFER
INSERT INTO LA_LOLA_ROTICERIA.CHOFER (NRO_LEGAJO, NOMBRE, APELLIDO, DNI, DIRECCION, TELEFONO, MAIL, FECHA_NACIMIENTO, COSTO_HORA)
	SELECT distinct CHOFER_NRO_LEGAJO, CHOFER_NOMBRE, CHOFER_APELLIDO, CHOFER_DNI, CHOFER_DIRECCION, CHOFER_TELEFONO, CHOFER_MAIL, CHOFER_FECHA_NAC, CHOFER_COSTO_HORA
	FROM gd_esquema.Maestra
	WHERE CHOFER_NRO_LEGAJO is not null

--VIAJE
INSERT INTO LA_LOLA_ROTICERIA.VIAJE (RECORIDO_ID, CAMION_ID, CHOFER_ID, FECHA_INICIO, FECHA_FIN, CONSUMO_COMBUSTIBLE)
	SELECT distinct (select r.ID 
					from LA_LOLA_ROTICERIA.RECORRIDO r 
					where r.CIUDAD_DESTINO = (select c.ID from LA_LOLA_ROTICERIA.CIUDAD c where c.NOMBRE = RECORRIDO_CIUDAD_DESTINO)
					and r.CIUDAD_ORIGEN = (select c.ID from LA_LOLA_ROTICERIA.CIUDAD c where c.NOMBRE = RECORRIDO_CIUDAD_ORIGEN) 
					and r.PRECIO_RECORRIDO = RECORRIDO_PRECIO 
					and r.PRECIO_RECORRIDO = RECORRIDO_PRECIO), 
					CAMION_PATENTE, CHOFER_NRO_LEGAJO, VIAJE_FECHA_INICIO, VIAJE_FECHA_FIN, VIAJE_CONSUMO_COMBUSTIBLE
	FROM gd_esquema.Maestra
	WHERE VIAJE_FECHA_FIN is not null

--TIPO_PAQUETE
INSERT INTO LA_LOLA_ROTICERIA.TIPO_PAQUETE (DESCRIPCION, ANCHO_MAXIMO, ALTO_MAXIMO, LARGO_MAXIMO, PESO_MAXIMO, PRECIO)
	SELECT distinct PAQUETE_DESCRIPCION, PAQUETE_ANCHO_MAX, PAQUETE_ALTO_MAX, PAQUETE_LARGO_MAX, PAQUETE_PESO_MAX, PAQUETE_PRECIO
	FROM gd_esquema.Maestra
	WHERE PAQUETE_DESCRIPCION IS NOT NULL

--PAQUETE
INSERT INTO LA_LOLA_ROTICERIA.PAQUETE ( VIAJE_ID, TIPO_ID, PRECIO_FINAL, CANTIDAD_PAQUETES)
	SELECT (select v.VIAJE_NRO
			from LA_LOLA_ROTICERIA.VIAJE v
			inner join LA_LOLA_ROTICERIA.RECORRIDO r on v.RECORIDO_ID = r.ID
			inner join LA_LOLA_ROTICERIA.CHOFER chof on v.CHOFER_ID = chof.NRO_LEGAJO
			inner join LA_LOLA_ROTICERIA.CAMION cam on v.CAMION_ID = cam.PATENTE
			where v.CONSUMO_COMBUSTIBLE = m.VIAJE_CONSUMO_COMBUSTIBLE
			and v.FECHA_INICIO = m.VIAJE_FECHA_INICIO
			and v.FECHA_FIN = m.VIAJE_FECHA_FIN
			and r.KM_RECORIDOS = m.RECORRIDO_KM
			and r.PRECIO_RECORRIDO = m.RECORRIDO_PRECIO
			and chof.NRO_LEGAJO = m.CHOFER_NRO_LEGAJO
			and cam.PATENTE = m.CAMION_PATENTE),
		  (select t_p.ID
		   from LA_LOLA_ROTICERIA.TIPO_PAQUETE t_p
		   where t_p.DESCRIPCION = m.PAQUETE_DESCRIPCION),
		   (m.PAQUETE_PRECIO * m.PAQUETE_CANTIDAD) + m.RECORRIDO_PRECIO,
		   m.PAQUETE_CANTIDAD
	FROM gd_esquema.Maestra m
	WHERE PAQUETE_DESCRIPCION IS NOT NULL

--MATERIAL
INSERT INTO LA_LOLA_ROTICERIA.MATERIAL (ID, DESCRIPCION, PRECIO)
	SELECT distinct MATERIAL_COD, MATERIAL_DESCRIPCION, MATERIAL_PRECIO
	FROM gd_esquema.Maestra
	WHERE MATERIAL_COD IS NOT NULL

--TIPO_TAREA
INSERT INTO LA_LOLA_ROTICERIA.TIPO_TAREA (TIPO)
	SELECT distinct TIPO_TAREA
	FROM gd_esquema.Maestra
	WHERE TIPO_TAREA IS NOT NULL

	
--TAREA
INSERT INTO LA_LOLA_ROTICERIA.TAREA (DESCRIPCION, CODIGO,TIPO_ID)
	SELECT distinct TAREA_DESCRIPCION, TAREA_CODIGO,tip.ID
	FROM gd_esquema.Maestra p
	LEFT JOIN LA_LOLA_ROTICERIA.TIPO_TAREA tip ON tip.TIPO = p.TIPO_TAREA
	WHERE TAREA_DESCRIPCION IS NOT NULL


--MATERIAL_X_TAREA
INSERT INTO LA_LOLA_ROTICERIA.MATERIAL_X_TAREA (TAREA_ID, MATERIAL_ID)
	SELECT distinct tarea.ID, material.ID
	FROM gd_esquema.Maestra maestra
	INNER JOIN LA_LOLA_ROTICERIA.MATERIAL material ON material.DESCRIPCION = maestra.MATERIAL_DESCRIPCION
	INNER JOIN LA_LOLA_ROTICERIA.TAREA tarea ON tarea.DESCRIPCION = maestra.TAREA_DESCRIPCION
	WHERE TAREA_DESCRIPCION IS NOT NULL

--ESTADO
INSERT INTO LA_LOLA_ROTICERIA.ESTADO (DESCRIPCION)
	SELECT distinct ORDEN_TRABAJO_ESTADO
	FROM gd_esquema.Maestra 
	WHERE ORDEN_TRABAJO_ESTADO IS NOT NULL


--ORDEN_TRABAJO
INSERT INTO LA_LOLA_ROTICERIA.ORDEN_TRABAJO (CAMION_ID, FECHA_GENERACION, ESTADO_ID)
	SELECT distinct CAMION_PATENTE, ORDEN_TRABAJO_FECHA, estado.ID
	FROM gd_esquema.Maestra p
	LEFT JOIN LA_LOLA_ROTICERIA.ESTADO estado ON estado.DESCRIPCION = p.ORDEN_TRABAJO_ESTADO
	WHERE ORDEN_TRABAJO_ESTADO IS NOT NULL


--TALLER
INSERT INTO LA_LOLA_ROTICERIA.TALLER (DIRECCION, TELEFONO, MAIL, NOMBRE, CIUDAD_ID)
	SELECT distinct TALLER_DIRECCION, TALLER_TELEFONO, TALLER_MAIL, TALLER_NOMBRE, city.ID
	FROM gd_esquema.Maestra p
	JOIN LA_LOLA_ROTICERIA.CIUDAD city ON city.NOMBRE = p.TALLER_CIUDAD
	WHERE TALLER_DIRECCION IS NOT NULL
	

--MECANICO
INSERT INTO LA_LOLA_ROTICERIA.MECANICO (NRO_LEGAJO, TALLER_ID, NOMBRE, APELLIDO, DNI, DIRECCION, TELEFONO, MAIL, FECHA_NACIMIENTO, COSTO_HORA)
	SELECT distinct 
		MECANICO_NRO_LEGAJO, taller.ID, MECANICO_NOMBRE, MECANICO_APELLIDO, MECANICO_DNI, 
		MECANICO_DIRECCION, MECANICO_TELEFONO, MECANICO_MAIL, MECANICO_FECHA_NAC, MECANICO_COSTO_HORA
	FROM gd_esquema.Maestra p
	LEFT JOIN LA_LOLA_ROTICERIA.TALLER taller ON taller.DIRECCION = p.TALLER_DIRECCION
	WHERE MECANICO_NRO_LEGAJO IS NOT NULL


--TAREA_ASIGNADA
INSERT INTO LA_LOLA_ROTICERIA.TAREA_ASIGNADA (MECANICO_ID, TAREA_ID, TIEMPO_DE_EJECUCION_ESTIMADO, FECHA_INICIO_ESTIMADA, ORDEN_TRABAJO_ID, FECHA_INICIO_REAL, FECHA_FIN, TIEMPO_DE_EJECUCION_REAL)
	SELECT distinct 
		MECANICO_NRO_LEGAJO, tarea.ID, TAREA_TIEMPO_ESTIMADO, TAREA_FECHA_INICIO_PLANIFICADO, ot.NRO, TAREA_FECHA_INICIO, TAREA_FECHA_FIN, DATEDIFF(day, TAREA_FECHA_INICIO, TAREA_FECHA_FIN)
	FROM gd_esquema.Maestra p
	LEFT JOIN LA_LOLA_ROTICERIA.TAREA tarea ON tarea.DESCRIPCION = p.TAREA_DESCRIPCION
	LEFT JOIN LA_LOLA_ROTICERIA.ORDEN_TRABAJO ot ON ot.FECHA_GENERACION = p.ORDEN_TRABAJO_FECHA and ot.CAMION_ID = p.CAMION_PATENTE
	WHERE MECANICO_NRO_LEGAJO IS NOT NULL
